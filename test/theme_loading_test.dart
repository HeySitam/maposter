...
void main() {
  ...
  group('bundled themes', () {
    test(
      'loadAll returns all 17 themes with ids injected from filenames',
      () async {
  ...
  group('custom themes on the engine', () {
    const config = MaposterConfig(userAgent: 'test/1.0 (test)');
    ...
    test('a new custom theme is appended (17 -> 18)', () async {
      final engine = MaposterEngine(config, customThemes: [custom('brand')]);
    ...
    test('a custom theme reusing a built-in id overrides it', () async {
      ...
      final engine = MaposterEngine(config, customThemes: [mine]);
      final themes = await engine.getAllThemes();
