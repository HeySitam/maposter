...
Map<String, dynamic> _mapFeatureToJson(MapFeature f) => {
      'rings': [
        for (final ring in f.rings)
          [for (final ll in ring) _latLonToJson(ll)],
      ],
      'type': f.type.name,
    };
...
class OverpassLocalDatasourceImpl implements OverpassLocalDatasource {
  ...
  Future<List<RoadSegment>?> getRoads(String key) async {
    ...
    return [for (final m in list) _roadSegmentFromJson(m as Map<String, dynamic>)];
  }

  @override
  Future<void> putRoads(String key, List<RoadSegment> roads) =>
      _cache.write(key, jsonEncode([for (final r in roads) _roadSegmentToJson(r)]));
  ...
  Future<List<MapFeature>?> getFeatures(String key) async {
    ...
    if (cached == null) return null;
    final list = jsonDecode(cached) as List;
    return [for (final m in list) _mapFeatureFromJson(m as Map<String, dynamic>)];
  }

  @override
  Future<void> putFeatures(String key, List<MapFeature> features) =>
      _cache.write(key, jsonEncode([for (final f in features) _mapFeatureToJson(f)]));
}
