...
/// Paints a finished map poster onto a [Canvas]: background, water, parks,
/// roads (batched by [RoadType]), gradient fades, and the city/country/
/// coordinate text labels. Use directly in a `CustomPaint`, or via
/// `MaposterEngine.buildPainter` / `MaposterEngine.exportPng`.
class MaposterPainter extends CustomPainter {
  const MaposterPainter({
  ...
  void paint(Canvas canvas, Size size) {
    canvas.clipRect(
      Offset.zero & size,
    ); // clip cover-mode overflow to poster bounds
  ...
  @override
  bool shouldRepaint(MaposterPainter old) =>
      old.mapData != mapData ||
      old.theme != theme ||
